...
func (pdc *DomainControllerImpl) GetUserInfo(c *gin.Context) {
	...
	tokenString := c.GetHeader("Authorization")
	GetUserDetails(tokenString, c)
	return
}

/*
custom function for getting UserInfo details
*/
func GetUserDetails(tokenString string, c *gin.Context) {
	...
	if response.StatusCode == http.StatusOK {
		...
		if err := json.Unmarshal(responseBody, &userSuccessresp); err != nil {
			...
			if err := json.Unmarshal(responseBody, &userFailuresresp); err != nil {
				c.JSON(http.StatusOK, gin.H{"error": "unmarshalling error during login return Jwt code in /Getuserdetails"})
				return
			}
		...
		//fmt.Println("API request failed with status code:", string(tokenResonse))
		//dtokenResonse := tokenResonse.tokenResonse
		/*dRoleInfo := userSuccessresp.Detail
		ref := reflect.ValueOf(dRoleInfo)
		dRoleId := reflect.Indirect(ref).FieldByName("RoleId")
		dRoleName := reflect.Indirect(ref).FieldByName("RoleName")
		fmt.Println("API request failed with status code:", dRoleName)
		userinfo := models.UserInfo{
			Token:               tokenString[len("Bearer "):],
			RoleName:            dRoleName.String(),
			RoleId:              dRoleId.Int(),
			RequiredPermissions: models.RoleMatrix[strings.ToLower(dRoleName.String())],
		}*/
